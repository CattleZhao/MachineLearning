...
if __name__ == '__main__':
    ...
    path = u'..\\8.Regression\\iris.data'
    data = pd.read_csv(path, header=None)
    x_prime = data[list(range(4))]
    y = pd.Categorical(data[4]).codes
    x_prime_train, x_prime_test, y_train, y_test = train_test_split(x_prime, y, random_state=1, test_size=0.3)
    ...
    for i, pair in enumerate(feature_paris):
        ...
        plt.scatter(x_test[pair[0]], x_test[pair[1]], c=y_test, s=20, edgecolors='k', cmap=cm_dark)
